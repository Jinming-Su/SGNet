...
class Decoder_lane(nn.Module):
    ...
    def forward(self, feat_low, feat_aspp):
        H, W = feat_aspp.size()[2:]
        feat_low = self.conv_low(feat_low)
        feat_low_down = F.interpolate(feat_low, (H, W), mode='bilinear',
                align_corners=True)
        feat_cat = torch.cat([feat_low_down, feat_aspp], dim=1)
...
def featuremap_2_heatmap(feature_map, channel):
    assert isinstance(feature_map, torch.Tensor)
    feature_map = feature_map.detach()
#     heatmap = feature_map[:,0,:,:]*0
#     for c in range(feature_map.shape[1]):
#         heatmap+=feature_map[:,c,:,:]
    heatmap = feature_map[:, channel,:,:]    
...
class LaneATTVP_lane_attenadd_fpn_more_conv_refine_laneseg_lane_level_linear_matrix_gaussian(nn.Module):
    def __init__(self,
                 backbone='resnet34',
                 ...
                 grid_pixel=20,
                 pixel_stride = 5):
        super(LaneATTVP_lane_attenadd_fpn_more_conv_refine_laneseg_lane_level_linear_matrix_gaussian, self).__init__()
        # Some definitions
...
#         self.feature_extractor = Resnet50(stride=self.stride)
        ...
        self.vp_base_anchors = self.gen_vp_base_anchors(angles, self.grid_pixel, pixel_stride, on_line)
#         import pdb
#         pdb.set_trace()
...
#             self.anchor_feat_channels, fmap_w, self.fmap_h)
        ...
        # Setup and initialize layers
        self.conv0 = ConvBNReLU(256, 256, 3, 1)
        self.conv1 = ConvBNReLU(256, self.anchor_feat_channels, 1, stride=1, padding=0)
        self.conv2 = ConvBNReLU(self.anchor_feat_channels, self.anchor_feat_channels, 3, 1)
        #self.conv1 = nn.Conv2d(backbone_nb_channels , self.anchor_feat_channels, kernel_size=1)
        self.cls_layer_0 = nn.Linear(self.anchor_feat_channels * self.fmap_h, 1024)
        self.reg_layer_0 = nn.Linear(self.anchor_feat_channels * self.fmap_h, 1024)
        
        self.cls_layer = nn.Linear(1024, 2)
        self.reg_layer = nn.Linear(1024, self.n_offsets + 1)
        self.relu = nn.ReLU(inplace=True)
        ...
        self.lane_decoder = Decoder_lane(2, low_chan=256)
...
#         pdb.set_trace()
        ...
        pred_vps = torch.where(torch.isnan(pred_vps), torch.tensor(0.0, device = pred_vps.device), pred_vps)
        lane_logits = self.lane_decoder(neck_features[0], feat_aspp)
        lane_probs = F.softmax(lane_logits, 1)
        
        lane_logits = F.interpolate(lane_logits, (self.img_h, self.img_w), mode='bilinear', align_corners=True)
             
        batch_features = batch_features + batch_features * lane_probs[:, 1:, :, :]
        batch_features = self.conv0(batch_features)
...
#         batch_anchor_features = torch.cat((attention_features, batch_anchor_features), dim=1)
        ...
        # Undo joining
        # Predict
        cls_logits = self.cls_layer_0(batch_anchor_features)
        cls_logits = self.relu(cls_logits)
        cls_logits=  self.cls_layer(cls_logits)
        reg = self.reg_layer_0(batch_anchor_features)
        reg = self.relu(reg)
    ...
    def draw(self, x, rpn_proposals = None, vp=None, conf_threshold=None, nms_thres=0, nms_topk=3000, lane_seg_pred=None, idx=None, vis_path = None, channel = None):
...
#         pdb.set_trace()
        ...
        pred_lane_probs = np.load(os.path.join(pred_seg_dir, '{}_probs.npy'.format(str(idx))))
        
        
        # 进行乘加的特征图可视化
        pred_lane_probs = torch.tensor(pred_lane_probs, device = batch_features.device)
        batch_features_for_visual =  F.interpolate(backbone_features[2], (590, 1640), mode='bilinear', align_corners=True)
#         import pdb
#         pdb.set_trace()
        batch_features_for_visual = batch_features_for_visual
    
        max_res = 0
        for c in range(batch_features_for_visual.shape[1]):
            response = (batch_features_for_visual[:, c, :, : ] * pred_lane_probs[:, 1, :, :]).sum()
            if response > max_res:
                max_res = response 
                max_c = c
        print(max_c)
#         import pdb
#         pdb.set_trace()
        batch_features_for_visual_add = batch_features_for_visual + batch_features_for_visual * pred_lane_probs[:, 1:, :, :]
        heatmap = featuremap_2_heatmap(batch_features_for_visual, channel = max_c)
        heatmap = np.uint8(255 * heatmap)
        heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        
        heatmap_add = featuremap_2_heatmap(batch_features_for_visual_add, channel = max_c)
        heatmap_add = np.uint8(255 * heatmap_add)
        heatmap_add = cv2.applyColorMap(heatmap_add, cv2.COLORMAP_JET)
...
#         batch_anchor_features = torch.cat((attention_features, batch_anchor_features), dim=1)
        ...
        proposals_list = self.nms(reg_proposals, reg_proposals, nms_thres, nms_topk, conf_threshold, anchors)
        #end_time = time()
        return proposals_list, vp_logits, pred_vps, lane_logits, _, heatmap, heatmap_add
    ...
    def loss(self, proposals_list, vp_logits, vp_preds, targets, vp_labels= None, lane_logits=None, lane_labels = None, h_paras = None, cls_loss_weight=10, lane_loss_weight=5, lane_level_loss_weight = 1, gaussian=True, base_offset_loss=0.5): 
...
#                 pdb.set_trace()
            ...
            #高斯形式的offset loss
            weighted_offset_mask = torch.ones_like(reg_target) * base_offset_loss
...
#                 add_weight = torch.exp(-0.5 * ( torch.pow((torch.linspace(1, 0, steps=self.n_offsets, dtype=torch.float32, device=target.device).repeat(reg_pred.shape[0], 1) - vp_pred[1]) / 0.25 , 2) +  torch.pow(( reg_target_norm  - vp_pred[0])/ 0.25, 2) ))
        ...
        loss =  reg_loss + cls_loss * cls_loss_weight + vp_loss * 5 + lane_loss * lane_loss_weight + lane_level_loss * lane_level_loss_weight

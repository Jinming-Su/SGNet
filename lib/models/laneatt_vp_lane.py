...
class LaneATTVP_lane(nn.Module):
    def __init__(self,
                 backbone='resnet34',
        ...
        super(LaneATTVP_lane, self).__init__()
        # Some definitions
...
#             self.anchor_feat_channels, fmap_w, self.fmap_h)
        ...
        self.conv1 = nn.Conv2d(backbone_nb_channels + 1, self.anchor_feat_channels, kernel_size=1)
    ...
    def forward(self, x, rpn_proposals = None, vp=None, conf_threshold=None, nms_thres=0, nms_topk=3000):
        ...
        batch_features = torch.cat((batch_features, lane_probs[:, 1:, :, :]), dim=1)
...
#         # Predict
    ...
    def loss(self, proposals_list, vp_logits, vp_preds, targets, vp_labels= None, lane_logits=None, lane_labels = None, cls_loss_weight=10): 
        ...
        lane_loss = self.vp_criteria(lane_logits, lane_labels)
        with torch.no_grad():
            gt_vps = []

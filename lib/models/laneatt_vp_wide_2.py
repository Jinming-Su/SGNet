...
class LaneATTVP_wide_2(nn.Module):
    def __init__(self,
                 backbone='resnet34',
        ...
        super(LaneATTVP_wide_2, self).__init__()
        # Some definitions
...
#             self.anchor_feat_channels, fmap_w, self.fmap_h)
        ...
        self.cls_layer = nn.Linear(2 * self.anchor_feat_channels * self.fmap_h * 5, 2)
        self.reg_layer = nn.Linear(2 * self.anchor_feat_channels * self.fmap_h * 5, self.n_offsets + 1)
        #self.attention_layer = nn.Linear(self.anchor_feat_channels * self.fmap_h, len(self.anchors) - 1)
        self.attention_layer = nn.Linear(self.anchor_feat_channels * self.fmap_h * 5, self.n_proposal - 1)
    ...
    def forward(self, x, rpn_proposals = None, vp=None, conf_threshold=None, nms_thres=0, nms_topk=3000):
        ...
        cut_xs_wide, invalid_mask_wide = self.compute_anchor_cut_indices_proposals_xs_wide_2(anchors, anchors_cut, 
        ...
        batch_anchor_features = self.cut_anchor_features_proposals_wide_2(batch_features, cut_xs_wide, invalid_mask_wide, self.cut_zs, self.cut_ys)
        # Join proposals from all images into a single proposals features batch
        batch_anchor_features = batch_anchor_features.view(-1, self.anchor_feat_channels * self.fmap_h * 5)
        ...
        attention_features = attention_features.reshape(-1, self.anchor_feat_channels * self.fmap_h * 5)
        batch_anchor_features = batch_anchor_features.reshape(-1, self.anchor_feat_channels * self.fmap_h * 5)
        ...
        #end_time = time()
        return proposals_list, vp_logits, pred_vps,_
...
#     def compute_anchor_cut_indices_proposals_xs_wide(self, anchors, anchors_cut, n_fmaps, fmaps_w, fmaps_h):
#         n_proposals = self.n_proposal 
#         anchors_cut = anchors_cut.reshape(-1, anchors_cut.shape[-1])
...
#         unclamped_xs_left = unclamped_xs - 1
#         unclamped_xs_right = unclamped_xs + 1
# #         unclamped_xs_wide = torch.stack((unclamped_xs, unclamped_xs_left, unclamped_xs_right),dim=2)
# #         import pdb
# #         pdb.set_trace()
#         cut_xs = torch.clamp(unclamped_xs, 0, fmaps_w - 1)
#         cut_xs = cut_xs.reshape(anchors.shape[0], -1, 1)
#         cut_xs_left = torch.clamp(unclamped_xs_left, 0, fmaps_w - 1)
#         cut_xs_left = cut_xs_left.reshape(anchors.shape[0], -1, 1)
#         cut_xs_right = torch.clamp(unclamped_xs_right, 0, fmaps_w - 1)
#         cut_xs_right = cut_xs_left.reshape(anchors.shape[0], -1, 1)
#         unclamped_xs = unclamped_xs.reshape(anchors.shape[0], n_proposals, n_fmaps, fmaps_h, 1)
#         invalid_mask = (unclamped_xs < 0) | (unclamped_xs > fmaps_w)
#         unclamped_xs_left = unclamped_xs_left.reshape(anchors.shape[0], n_proposals, n_fmaps, fmaps_h, 1)
#         invalid_mask_left = (unclamped_xs_left < 0) | (unclamped_xs_left > fmaps_w)
#         unclamped_xs_right = unclamped_xs_right.reshape(anchors.shape[0], n_proposals, n_fmaps, fmaps_h, 1)
#         invalid_mask_right = (unclamped_xs_right < 0) | (unclamped_xs_right > fmaps_w)
#         return cut_xs, invalid_mask, cut_xs_left, invalid_mask_left, cut_xs_right, invalid_mask_right
    ...
    def compute_anchor_cut_indices_proposals_xs_wide(self, anchors, anchors_cut, n_fmaps, fmaps_w, fmaps_h):
        ...
        unclamped_xs_wide = torch.cat((unclamped_xs_left ,unclamped_xs, unclamped_xs_right), dim=1).reshape(-1, 1)
        cut_xs_wide = torch.clamp(unclamped_xs_wide, 0, fmaps_w - 1)
        cut_xs_wide = cut_xs_wide.reshape(anchors.shape[0], -1, 3)
        unclamped_xs_wide = unclamped_xs_wide.reshape(anchors.shape[0], n_proposals, n_fmaps, fmaps_h * 3, 1)
        invalid_mask_wide = (unclamped_xs_wide < 0) | (unclamped_xs_wide > fmaps_w)
...
#         unclamped_xs_right = unclamped_xs_right.reshape(anchors.shape[0], n_proposals, n_fmaps, fmaps_h, 1)
#         invalid_mask_right = (unclamped_xs_right < 0) | (unclamped_xs_right > fmaps_w)
        return cut_xs_wide, invalid_mask_wide

    def compute_anchor_cut_indices_proposals_xs_wide_2(self, anchors, anchors_cut, n_fmaps, fmaps_w, fmaps_h):
        n_proposals = self.n_proposal 
        anchors_cut = anchors_cut.reshape(-1, anchors_cut.shape[-1])
        ...
        unclamped_xs_left = unclamped_xs - 1
        unclamped_xs_left_left = unclamped_xs - 2
        unclamped_xs_right = unclamped_xs + 1
        unclamped_xs_right_right = unclamped_xs + 2
        
        unclamped_xs_wide = torch.cat((unclamped_xs_left_left, unclamped_xs_left ,unclamped_xs, unclamped_xs_right, unclamped_xs_right_right), dim=1).reshape(-1, 1)
        cut_xs_wide = torch.clamp(unclamped_xs_wide, 0, fmaps_w - 1)
        cut_xs_wide = cut_xs_wide.reshape(anchors.shape[0], -1, 5)
        unclamped_xs_wide = unclamped_xs_wide.reshape(anchors.shape[0], n_proposals, n_fmaps, fmaps_h * 5, 1)
        invalid_mask_wide = (unclamped_xs_wide < 0) | (unclamped_xs_wide > fmaps_w)
...
#         invalid_mask_right = (unclamped_xs_right < 0) | (unclamped_xs_right > fmaps_w)
    ...
    def cut_anchor_features_proposals_wide(self, features, cut_xs_wide, invalid_mask_wide, cut_zs, cut_ys):
        # definitions
        batch_size = features.shape[0]
        n_proposals = self.n_proposal  
        n_fmaps = features.shape[1]
        batch_anchor_features = torch.zeros((batch_size, n_proposals, n_fmaps, self.fmap_h * 3, 1), device=features.device)
        
        # actual cutting
        #cut_xs_wide = torch.cat((cut_xs_left, cut_xs, cut_xs_right), dim=2)
        for batch_idx, img_features in enumerate(features):
            rois = img_features[cut_zs, cut_ys, cut_xs_wide[batch_idx]].view(n_proposals, n_fmaps, self.fmap_h * 3, 1)
            rois[invalid_mask_wide[batch_idx]] = 0
            batch_anchor_features[batch_idx] = rois

        return batch_anchor_features

    def cut_anchor_features_proposals_wide_2(self, features, cut_xs_wide, invalid_mask_wide, cut_zs, cut_ys):
        # definitions
        batch_size = features.shape[0]
        n_proposals = self.n_proposal  
        n_fmaps = features.shape[1]
        batch_anchor_features = torch.zeros((batch_size, n_proposals, n_fmaps, self.fmap_h * 5, 1), device=features.device)
        
        # actual cutting
        #cut_xs_wide = torch.cat((cut_xs_left, cut_xs, cut_xs_right), dim=2)
        for batch_idx, img_features in enumerate(features):
            rois = img_features[cut_zs, cut_ys, cut_xs_wide[batch_idx]].view(n_proposals, n_fmaps, self.fmap_h * 5, 1)
            rois[invalid_mask_wide[batch_idx]] = 0
            batch_anchor_features[batch_idx] = rois

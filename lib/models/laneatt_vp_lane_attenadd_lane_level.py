...
class LaneATTVP_lane_attenadd_lane_level(nn.Module):
    def __init__(self,
                 backbone='resnet34',
        ...
        super(LaneATTVP_lane_attenadd_lane_level, self).__init__()
        # Some definitions
...
#                 pdb.set_trace()
                ...
                Y = torch.stack((transformed[:, 1] * transformed[:, 1], transformed[:, 1], torch.ones(transformed.shape[0], device = proposal_xy.device)), dim=1)
                x_1 = transformed[:, 0:1]
                w = (torch.inverse((Y.T).mm(Y)).mm(Y.T)).mm(x_1)
                ws.append(w)
            if len(ws) > 1:    
                ws = torch.stack(ws)
                lane_level_loss += smooth_l1_loss(ws[:, :2], torch.roll(ws[:, :2], 1))
            # Loss calc

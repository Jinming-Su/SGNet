...
class LaneATTVP_wide_noatten(nn.Module):
    def __init__(self,
                 backbone='resnet34',
        ...
        super(LaneATTVP_wide_noatten, self).__init__()
        # Some definitions
...
#             self.anchor_feat_channels, fmap_w, self.fmap_h)
        ...
        self.cls_layer = nn.Linear(self.anchor_feat_channels * self.fmap_h * 3, 2)
        self.reg_layer = nn.Linear(self.anchor_feat_channels * self.fmap_h * 3, self.n_offsets + 1)
        #self.attention_layer = nn.Linear(self.anchor_feat_channels * self.fmap_h, len(self.anchors) - 1)
        self.attention_layer = nn.Linear(self.anchor_feat_channels * self.fmap_h * 3, self.n_proposal - 1)
    ...
    def forward(self, x, rpn_proposals = None, vp=None, conf_threshold=None, nms_thres=0, nms_topk=3000):
        ...
        cut_xs_wide, invalid_mask_wide = self.compute_anchor_cut_indices_proposals_xs_wide(anchors, anchors_cut, 
        ...
        batch_anchor_features = self.cut_anchor_features_proposals_wide(batch_features, cut_xs_wide, invalid_mask_wide, self.cut_zs, self.cut_ys)
        # Join proposals from all images into a single proposals features batch
        batch_anchor_features = batch_anchor_features.view(-1, self.anchor_feat_channels * self.fmap_h * 3)
...
#         attention_features = attention_features.reshape(-1, self.anchor_feat_channels * self.fmap_h * 3)
#         batch_anchor_features = batch_anchor_features.reshape(-1, self.anchor_feat_channels * self.fmap_h *3)

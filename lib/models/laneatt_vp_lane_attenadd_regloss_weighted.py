...
class LaneATTVP_lane_attenadd_regloss_weighted(nn.Module):
    def __init__(self,
                 backbone='resnet34',
        ...
        super(LaneATTVP_lane_attenadd_regloss_weighted, self).__init__()
        # Some definitions
...
#         batch_anchor_features = torch.cat((attention_features, batch_anchor_features), dim=1)
    ...
    def loss(self, proposals_list, vp_logits, vp_preds, targets, vp_labels= None, lane_logits=None, lane_labels = None, cls_loss_weight=10, gaussian=True, base_offset_loss=1.0): 
        ...
        total_positives = 0
        vp_preds_norm = vp_preds / torch.tensor([self.img_w, self.img_h], device =vp_logits.device)
        for (proposals, anchors, _, _), target, vp_pred in zip(proposals_list, targets, vp_preds_norm):
...
#                 add_weight = torch.exp(-0.5 * ( torch.pow((torch.linspace(1, 0, steps=self.n_offsets, dtype=torch.float32, device=target.device).repeat(reg_pred.shape[0], 1) - vp_pred[1]) / 0.25 , 2) +  torch.pow(( reg_target_norm  - vp_pred[0])/ 0.25, 2) ))
                weighted_offset_mask[:, 1:] += torch.exp(-0.5 * ( torch.pow((torch.linspace(1, 0, steps=self.n_offsets, dtype=torch.float32, device=target.device).repeat(reg_pred.shape[0], 1) - vp_pred[1]) / 0.25 , 2) +  torch.pow((reg_target_norm - vp_pred[0])/ 0.25, 2) ))
...
#             pdb.set_trace()
        ...
        loss =  reg_loss + cls_loss * cls_loss_weight + vp_loss * 5 + lane_loss * 5

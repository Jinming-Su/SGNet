...
class LaneATTVP_lane_attenadd_lane_level_linear_matrix_cut(nn.Module):
    def __init__(self,
                 backbone='resnet34',
        ...
        super(LaneATTVP_lane_attenadd_lane_level_linear_matrix_cut, self).__init__()
        # Some definitions
...
#         batch_anchor_features = torch.cat((attention_features, batch_anchor_features), dim=1)
    ...
    def loss(self, proposals_list, vp_logits, vp_preds, targets, vp_labels= None, lane_logits=None, lane_labels = None, h_paras = None, cls_loss_weight=10, lane_level_loss_weight = 1, cut_thresh = 0.1): 
...
#                 pdb.set_trace()
                ...
                mask1 = torch.abs(alpha_r - alpha_c) <  0
                mask2 = torch.abs(alpha_r - alpha_c) > cut_thresh
                alpha_r = alpha_r + cut_thresh * mask1 - cut_thresh * (~mask1)
                lane_level_loss += smooth_l1_loss_sum(alpha_r * mask2, alpha_c * mask2) / (number_lane * (number_lane - 1) / 2) 
            # Loss calc

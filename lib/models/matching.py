...
#     pdb.set_trace()
    ...
    proposals_copy = proposals.clone().detach()
    targets_copy  =targets.clone().detach()
    ...
    targets = torch.cat(num_proposals * [targets])  # ssapplying this 2 times on [c, d] gives [c, d, c, d]
    ...
    # TODO 为什么个别值y_start会不在0到1之间
    starts = torch.max(targets_starts, proposals_starts).round().long().clamp(max=72)
    ...
    if ((5+starts) >= valid_offsets_mask.shape[1]).sum() > 0:
        print(proposals_copy)
        print(targets_copy)
        import pdb
        pdb.set_trace()
    try:

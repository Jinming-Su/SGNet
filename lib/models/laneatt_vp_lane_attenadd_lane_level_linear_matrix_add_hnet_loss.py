...
class LaneATTVP_lane_attenadd_lane_level_linear_matrix_add_hnetloss(nn.Module):
    def __init__(self,
                 backbone='resnet34',
        ...
        super(LaneATTVP_lane_attenadd_lane_level_linear_matrix_add_hnetloss, self).__init__()
        # Some definitions
...
#         batch_anchor_features = torch.cat((attention_features, batch_anchor_features), dim=1)
    ...
    def loss(self, proposals_list, vp_logits, vp_preds, targets, vp_labels= None, lane_logits=None, lane_labels = None, h_paras = None, cls_loss_weight=10, lane_level_loss_weight = 1, hnet_loss_weight = 1): 
        ...
        for (proposals, anchors, _, _), target, h_matrix in zip(proposals_list, targets, h_paras):
...
#                 pdb.set_trace()
        ...
        loss =  reg_loss + cls_loss * cls_loss_weight + vp_loss * 5 + lane_loss * 5 + lane_level_loss * lane_level_loss_weight + hnet_loss * hnet_loss_weight 
        return loss, {'vp_loss': vp_loss, 'vp_dis': vp_dis, 'lane_loss': lane_loss, 'lane_level_loss' : lane_level_loss, 'hnet_loss': hnet_loss, 'cls_loss': cls_loss, 'reg_loss': reg_loss, 'batch_positives': total_positives}

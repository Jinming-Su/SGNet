...
class LaneATTVP_lane_attenadd_nonlocal(nn.Module):
    def __init__(self,
                 backbone='resnet34',
        ...
        super(LaneATTVP_lane_attenadd_nonlocal, self).__init__()
        # Some definitions
...
#             self.anchor_feat_channels, fmap_w, self.fmap_h)
        ...
        self.non_local_block1 = NonLocalBlock(backbone_nb_channels)
        self.conv2 = ConvBNReLU(backbone_nb_channels, backbone_nb_channels, 3, 1)
        self.conv3 = ConvBNReLU(backbone_nb_channels, backbone_nb_channels, 1, stride=1, padding=0)
        self.conv4 = ConvBNReLU(backbone_nb_channels, backbone_nb_channels, 3, 1)
    ...
    def forward(self, x, rpn_proposals = None, vp=None, conf_threshold=None, nms_thres=0, nms_topk=3000):
        ...
        batch_features = self.non_local_block1(batch_features)
        batch_features = self.conv2(batch_features)
        batch_features = self.conv3(batch_features)
        batch_features = self.conv4(batch_features)
        #消失点计算

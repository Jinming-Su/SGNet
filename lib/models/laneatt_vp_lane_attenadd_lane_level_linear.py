...
class LaneATTVP_lane_attenadd_lane_level_linear(nn.Module):
    def __init__(self,
                 backbone='resnet34',
        ...
        super(LaneATTVP_lane_attenadd_lane_level_linear, self).__init__()
        # Some definitions
...
#                 pdb.set_trace()
            ...
            if len(ws) > 1:    
                ws = torch.stack(ws)
                lane_level_loss += smooth_l1_loss(ws[:, :1], torch.roll(ws[:, :1], 1))
            # Loss calc

...
#from mmdet.ops import bbox_dis

@IOU_CALCULATORS.register_module()
class Dis(object):
    """line S Calculator."""

    def __call__(self, bboxes1, bboxes2, pad_shape, mode='iou', is_aligned=False):
        """Calculate IoU between 2D bboxes.

        Args:
            bboxes1 (Tensor): bboxes have shape (m, 4) in <x1, y1, w1, angle>
                format, or shape (m, 5) in <x1, y1, w1, angle, score> format.
        ...
        Returns:
            similariry(Tensor): shape (m, n, 2) if is_aligned == False else shape (m, 1, 2)
            2代表面积与w差值分量
        """
        
#         import pdb
#         pdb.set_trace()
        assert bboxes1.size(-1) in [0, 4, 5, 10]
        assert bboxes2.size(-1) in [0, 4, 5, 10]
        if bboxes2.size(-1) == 5:
            bboxes2 = bboxes2[..., :4]
        if bboxes1.size(-1) == 5:
            bboxes1 = bboxes1[..., :4]
        if bboxes1.size(-1) == 10:
            bboxes1 = bboxes1[..., :4]
        w_gap = torch.abs(bboxes1[: ,2].unsqueeze(1) - bboxes2[: ,2].unsqueeze(0))
        w_gap1 = w_gap / bboxes1[: ,2].unsqueeze(1)
        w_dis =(w_gap1> 1/3).float()
#         import pdb
#         pdb.set_trace()
        #print('w_dis', w_dis)
        #         return 1 - bbox_dis(bboxes1, bboxes2, pad_shape) - \
#     ((torch.abs(bboxes1[: ,2].unsqueeze(1) - bboxes2[: ,2].unsqueeze(0)) / bboxes1[: ,2].unsqueeze(1)) > 1/3).float()
        return 1 - bbox_dis(bboxes1, bboxes2, pad_shape) - w_dis

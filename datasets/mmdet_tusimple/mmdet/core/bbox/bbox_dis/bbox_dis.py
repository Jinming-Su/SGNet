...
#分别输入两条线的两个点坐标得到他们的bbox_dis
def bbox_dis_points(a_points, b_points, pad_shape):
    """
    计算anchor和gt的面积量
    a_points: Tensor with shape (M, 2, (x, y))
    b_points: Tensor with shape (N, 2, (x, y))
    """
    H, W = pad_shape
    M = len(a_points)
    N = len(b_points)
...
def get_roi2align(bboxes, H, W):
    """
    bboxes (N, (x, y, w, angle))
    """
    inter_p,_,_,_ = get_inter_with_border_mul(bboxes, H, W)
    roi2align_x_y = (bboxes[:, :2] + inter_p) / 2 
    roi2align_h = F.pairwise_distance(bboxes[:, :2], inter_p, keepdim = True)
    mask1 = bboxes[:, 3:4] > 45
    #[0, pi/4]对应[0, -pi/4]   (pi/4, pi)对应（3/4pi, 0)
    #TODO 检查角度的转换和边界条件
    trans_angle = (180-bboxes[:, 3:4]) * mask1 + (-bboxes[:, 3:4]) * (~mask1)
    roi2align = torch.cat((roi2align_x_y, bboxes[:, 2:3], roi2align_h,  trans_angle), dim=1)

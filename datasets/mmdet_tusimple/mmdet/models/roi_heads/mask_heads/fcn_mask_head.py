...
#         pdb.set_trace()
    ...
    def get_targets(self, sampling_results, pos_roi2aligns, gt_masks, rcnn_train_cfg):
        pos_assigned_gt_inds = [
            res.pos_assigned_gt_inds for res in sampling_results
        ]
        #mask_targets改成
        mask_targets = mask_target(pos_roi2aligns, pos_assigned_gt_inds,
...
#     pdb.set_trace()
    ...
    #start = time.time()
    w_gap = (x_center - img_x)*torch.cos(theta) - (y_center - img_y)*torch.sin(theta)
    h_gap = (x_center - img_x)*torch.sin(theta) + (y_center - img_y)*torch.cos(theta)
    norm_x = w_gap / w * 2
    norm_y = h_gap / h * 2

...
#         normal_init(self.line_fc_regress, std=0.01)
    ...
    def forward_single(self, x, lines):
        ...
        H, W = x.shape[-2:]
        x_inds = torch.arange(W, device = x.device).long()
        rpn_bbox_pred_list = []
        rpn_cls_score_list = []
        #对不同的线进行不同的处理
        for i in range(x.size(0)):
            y_inds = ((x_inds * (lines[i, 1] - lines[i, 0]) * H / W + H * lines[i, 0]).round()).clamp(0, H - 1).long()
    ...
    def loss(self,
        ...
        return dict(line_loss=line_loss, loss_cls=losses_cls, loss_bbox=losses_bbox)
...
#         exit()   
    ...
    def get_bboxes(self,
        ...
        mlvl_anchors = self.anchor_generator.grid_anchors_on_line(
            featmap_sizes, lines[0], device=device)
    ...
    def _get_bboxes_single(self,
        ...
        #（N, 4), (x, y, w, h)
        dets, keep = lane_batched_nms(proposals, scores, ids, nms_cfg, img_shape = img_shape[:2])

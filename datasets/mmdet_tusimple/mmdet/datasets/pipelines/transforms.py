...
class LaneResize(object):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                ...
                img, w_scale, h_scale = mmcv.imresize(
                    ...
                    backend=self.backend)
            results[key] = img
            #假设keep_ratio, 角度的scale_factor维持不变
            scale_factor = np.array([w_scale, h_scale, w_scale, 1],
                                    dtype=np.float32)
    ...
    #CCTODO 暂时假设 keep_ratio, 先不考虑角度变换
    def _resize_bboxes(self, results):
        ...
        for key in results.get('bbox_fields', []):
            bboxes = results[key]
            #x,y,w乘scale_factor
            bboxes[:, :3] = bboxes[:, :3] * (results['scale_factor'][:3])
            bboxes[:, 0] = np.clip(bboxes[:, 0], 0, img_shape[1])
            bboxes[:, 1] = np.clip(bboxes[:, 1], 0, img_shape[0])
            results[key] = bboxes
        # img_shape = results['img_shape']
        # for key in results.get('bbox_fields', []):
    ...
    #CCTODO anno里的line是归一化好还是非归一化好
    def _resize_line(self, results):
        img_shape = results['img_shape']
        for key in results.get('line_fields', []):
            results[key] = results[key] * results['scale_factor'][1]
            results[key] = np.clip(results[key], 0, img_shape[0])
...
class LanePad(object):
    ...
    #在pad里把normalize做了
    def _pad_line(self, results):
        for key in results.get('line_fields', []):
            line = results[key]
            pad_line = line / results['pad_shape'][0]
            results[key] = pad_line
...
#             flipped[..., 3::4] = h - bboxes[..., 1::4]
    ...
    def line_flip(self, line, img_shape=None, direction=None):
        if direction == 'horizontal':
            flipped = line[1::-1]
        else:
            raise ValueError(f"Invalid flipping direction '{direction}'")
